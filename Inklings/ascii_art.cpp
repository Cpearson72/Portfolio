...
void clearTerminal() {
    std::cout << "\033[2J\033[3J\033[H" << std::flush; // Clears everything, including scrollback
}
...
void drawState(int numLiveThreads, int redLevel, int greenLevel, int blueLevel) {
    ...
    print("Last successful keyboard input: ", lastCommand);
...
void myKeyboard(unsigned char c) {
	...
	switch (c) {
        ...
        case '<':
            slowdownProducers();
            lastCommand = "Slow down producers";
            break;
            
        // speed up the production of ink for the producer threads
        case '>':
            speedupProducers();
            lastCommand = "Speed up producers";
            break;
            
		// test red ink up/down
		case 'r':
            lastCommand = "Refill red ink";
        ...
        case '+':
            if (inklingSleepTime - 100000 > 0) {
                ...
                lastCommand = "Make inklings move faster";
            }
        ...
        case '-':
            if (inklingSleepTime + 100000 < 2147483647) {
                inklingSleepTime += 100000;
                lastCommand = "Make inklings move slower";
            }
            break;
		...
		case 'g':
            ...
            lastCommand = "Refill green ink";
			break;

		// test blue ink up/down
		case 'b':
			ok = refillBlueInk(MAX_ADD_INK);
            lastCommand = "Refill blue ink";
			break;
